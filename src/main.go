...
type Boid struct {
	ID    string  `json:"id"`
	X     float64 `json:"x"`        // Canvas X (-1 to 1)
	Y     float64 `json:"y"`        // Canvas Y (-1 to 1)
	Angle float64 `json:"rotation"` // Canvas Rotation (度数 0-360)
	Speed float64 `json:"-"`        // 内部計算用速度
	Vx    float64 `json:"-"`        // 速度ベクトルX
	Vy    float64 `json:"-"`        // 速度ベクトルY
}
